...
- `internal/stage`: the stage directory contract (`.gobag/stage/` —
  plan.json, HANDOFF.md, series.json with series id / sequence /
  timestamps). Load, validate, atomic rewrite (write-temp-rename inside
  the stage dir; no partial stage on a crash).
- `gobag stage init` (from a plan or walk), `stage refresh` (mechanical:
  re-resolve refs/branches/dirty for every source in the staged plan;
  headless-safe), `stage status` (handoff age, refs moved since staged,
  base drift — reuse gitops.RemoteReality and the reconcile advisory
  code, do not fork it).
- `gobag seal [-o]`: staged plan → the existing pack pipeline (verify,
...
- Tests: stage refresh idempotent; status flags each staleness kind
  (reuse the FeatureBranch/AdvanceRemote fixtures); seal output restores
  identically to a pack of the same plan; concurrent-edit warning
  (stage mtime changed under the writer); hooks JSON is valid and the
  commands run headless in an empty environment.

**Done when:** a workspace with a live stage survives this sequence with
nothing lost: stage init → work → mechanical refresh → simulated
compaction (new session, no memory) → narrative update that provably
read the staged handoff first → seal → restore on a clean root reports
the same thread.
